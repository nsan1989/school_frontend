...
export default function Banner() {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        ...
        const desktop_images = data.msg.map(
          (item) => `${baseUrl}${item.desktop_format}`
        );

        const mobile_images = data.msg.map(
          (item) => `${baseUrl}${item.mobile_format}`
        );
        setDesktopImage(desktop_images);
        setMobileImage(mobile_images);
      } catch (error) {
  ...
  return (
    <>
      <div className="desktopWrapper d-none d-md-flex"
